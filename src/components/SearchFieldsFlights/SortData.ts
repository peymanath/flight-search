...
const initialDataPlace: FlightResponse[] = [
    {
        resultType: 'airport',
        countryCode: 'IN',
        countryNames: [
...
const makeDataPlace = initialDataPlace.map(({ resultType, countryNames, cities }) => {
...
dataPlace = dataPlace.flatMap((item: any) => (Array.isArray(item) ? item : [item]));
